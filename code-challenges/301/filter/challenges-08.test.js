...
const filterStringsWithVowels = ((arr) => arr.filter((val) => val.match(/[aeiou]/)));
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  let stat = arr.filter((obj) => obj.baseStat > minBaseStat);
  return stat;
};
...
const getCharactersWithoutChildren = (arr) => {
  let smallFam = arr.filter((family) => !family.children);
  return smallFam;
};
...
const evenOddNumericValues = (arr) => {
  let oddOrEven = [];
  let numbers = arr.filter((num) => Number.isInteger(num));
  numbers.map((num) => {
    if(num % 2 === 0) {
      oddOrEven.push('even');
    } else {
