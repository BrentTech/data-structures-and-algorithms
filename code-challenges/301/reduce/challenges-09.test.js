...
const countNumberOfElements = (arr) => {
  let totalEle = arr.reduce( (currentCount) => {
    currentCount++;
    return currentCount;
  },0);
  return totalEle;
...
const countNumberOfChildren = (arr) => {
  return arr.reduce((numChildren, family) => {
    if (family.children) {
      numChildren = numChildren + family.children.length;
    }
    return numChildren;
  },0);
};
...
const calculateAverage = (arr) => {
  let calcAv =  arr.reduce((acc, val) => {
    acc.sum = acc.sum + val;
    acc.count++;
    return acc;
  }, {count: 0, sum: 0,});
  return calcAv.sum / calcAv.count;
};
...
const extractStat = (statName, arr) => {
  return arr.reduce((selectedStat, statBlock) => {
    if (statBlock.stat.name === statName) {
      selectedStat = statBlock;
    }
    return selectedStat;
  }, null);
};
