...
const mapTwoToThe = (arr) => {
  let powerOf = arr.map( val => Math.pow(2, val));
  return powerOf;
};
...
const charCode = (arr) => {
  let charCodeResult = arr.map( val => val.charCodeAt());
...
const extractAbilities = (arr) => {
  let abilitiesArray = arr.map( data => data.ability.name);
...
const extractStats = (arr) => {
  let statsArray = [];
  arr.map( (data) => {
    // let statName = data.stat.name;
    // let statNumber = (parseInt(data.effort) + parseInt(data.baseStat));
    // let statObj = 
    statsArray.push({name: data.stat.name, total: (parseInt(data.effort) + parseInt(data.baseStat))});
  });
  return statsArray;
};
